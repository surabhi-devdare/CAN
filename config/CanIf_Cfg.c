...
#define CANIF_NUM_CHANNELS 1

static CanIf_RxPduType CanIf_RxPdus[CANIF_NUM_CHANNELS][CANIF_MAX_RX_PDU];
static CanIf_TxPduType CanIf_TxPdus[CANIF_NUM_CHANNELS];
static CanIf_CtrlContextType CanIf_CtrlContexts[CANIF_NUM_CHANNELS];
static CanIf_CtrlConfigType CanIf_CtrlConfigs[CANIF_NUM_CHANNELS];
...
INITIALIZER(_canif_init) {
  int i, j;
  for (i = 0; i < (int)ARRAY_SIZE(CanIf_CtrlContexts); i++) {
...
void CanIf_CanTpReconfig(uint8_t Channel, CanTp_ParamType *params) {
  if (Channel < CANIF_NUM_CHANNELS) {
    ...
    CanIf_RxPdus[Channel][0].mask = 0x7FF;
    CanIf_RxPdus[Channel][0].hoh = 0;
